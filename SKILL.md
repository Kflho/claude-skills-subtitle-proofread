...
对于维基/萌娘/Fandom 有详细资料的著名作品，**首次启动时尝试 Web 搜索预生成名词表**：

```
1. Web 搜索 "{作品名} キャラクター 一覧 日本語"
2. Web 搜索 "{作品名} 1963 characters list Japanese names"
3. 提取角色名/地名/术语 → reports/proper-nouns.md
```

生成后的名词表用于：
- Whisper 转录后对照：专名是否写对
- 全剧集名词统一：同一个角色在所有集中写法一致
- 标记未知词：不在表中的标记待查

**无名资料的小众作品**：跳过预搜索，校对时发现专名手动追加到名词表。
