...
def step_deliver(project_dir, lang, processed_episodes=None, is_full_run=True,
    ...
    """Layer 2 (unified): Human review delivery or checklist application.

    By default (after auto-fix), checklists are already generated by
    Fixer.run_auto() as part of step_fix_episodes. This step now either:

    - Without --apply-checklist: prints where to find the generated
      checklists and how to apply them.
    - With --apply-checklist: parses filled checklists for each
      processed episode and applies corrections to SRT files.

    Args:
        processed_episodes: list of episode IDs processed in this run
        apply_checklist: if True, apply filled checklists instead of generating
    """
    review_dir = os.path.join(project_dir, 'reports', 'manual-review')

    if apply_checklist:
        # Apply filled checklists
        if not processed_episodes:
            # Find all checklists in review dir
            processed_episodes = []
            if os.path.isdir(review_dir):
                for f in os.listdir(review_dir):
                    m = re.match(r'(EP\d{3})_checklist\.md', f)
                    if m:
                        processed_episodes.append(m.group(1))
        ...
        for ep in sorted(processed_episodes):
            ...
            if not os.path.exists(checklist):
                print(f'[deliver] No checklist for {ep} — skipping.', file=sys.stderr)
                continue
            fixer = Fixer(ep, project_dir)
            applied = fixer.apply(checklist)
            total_applied += applied
            print(f'[deliver] {ep}: {applied} corrections applied', file=sys.stderr)

        print(f'\n[deliver] Total: {total_applied} corrections applied across '
              f'{len(processed_episodes)} episodes', file=sys.stderr)
        return total_applied > 0
    else:
        # Just print where to find checklists
        if os.path.isdir(review_dir):
            checklists = [f for f in os.listdir(review_dir)
                         if f.endswith('_checklist.md')]
            if checklists:
                print(f'\n[deliver] {len(checklists)} checklists ready for review:',
                      file=sys.stderr)
                for cl in sorted(checklists):
                    print(f'  {review_dir}/{cl}', file=sys.stderr)
                print(f'\n[deliver] After filling in corrections, run:',
                      file=sys.stderr)
                print(f'  python scripts/run_all.py --lang {lang} --apply-checklist',
                      file=sys.stderr)
                return True

        print('[deliver] No checklists generated — all episodes are clean.',
              file=sys.stderr)
        return True
