...
# Common Japanese words that happen to be katakana — NOT proper nouns
_COMMON_KATAKANA = frozenset({
    # Basic loanwords
    'ドア', 'パック', 'テスト', 'メモ', 'データ', 'タイプ', 'レベル',
    'モデル', 'システム', 'プログラム', 'サービス', 'ケース', 'グループ',
    'チーム', 'クラス', 'ルール', 'コード', 'イメージ', 'デザイン',
    'コピー', 'チェック', 'リスト', 'ファイル', 'メッセージ', 'レポート',
    'サポート', 'プロジェクト', 'マシン', 'ライン', 'ポイント', 'ボタン',
    'スイッチ', 'パネル', 'ケーブル', 'エネルギー', 'スピード', 'バランス',
    'コントロール', 'センター', 'エリア', 'ゾーン', 'スペース', 'ホール',
    'ルーム', 'ハウス', 'カード', 'キー', 'ロック', 'ベル', 'サイン',
    'マーク', 'パパ', 'ママ', 'ボーイ', 'ガール', 'ベビー',
...
def build_glossary(term_freq, min_freq=_MIN_FREQ):
    ...
    for word, count in term_freq.items():
        ...
        if is_katakana and not is_kanji:
            ...
            if word not in _COMMON_KANJI:
                kanji_terms.append((word, count))

    # Group katakana by spelling variants
    katakana_groups = group_katakana_variants(katakana_terms)
...
def main():
    ...
    # Build glossary
    glossary = build_glossary(term_freq, min_freq=args.min_freq)
