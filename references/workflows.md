...
### Whisper 幻觉重复
音乐/噪声段 Whisper 会产生连续多条相同或高度相似的 cue。
- ❌ 切片重跑 Whisper **无效**（根因是音频本身触发幻觉，重跑只换一组幻觉）
- ✅ 正确做法：翻译阶段告知 LLM 这是 bug，照常翻译每条，最终人工审查决定保留哪条
- 🔧 检测可用（`fix_repeated_cues.py` 的相似度扫描），修复不可用
